...
export async function loader({ request, params }: LoaderFunctionArgs) {
  try {
    ...
    const groupedProducts = products.reduce(
      (result: any, product, currentIndex) => {
            ...
            <Card padding={"5"}>
              <HorizontalStack blockAlign="center" gap={"5"}>
                ...
                <Card>
                  ...
                  <Text as={"h6"}>{product.variant_title}</Text>
                </Card>
                <Text as={"p"}>
                  {product.total}&nbsp;
                  {product.total > 1 ? "days data" : "day data"}
                </Text>
              </HorizontalStack>
            </Card>
            <HorizontalStack blockAlign="center" gap={"5"}>
              <Card padding={"5"}>
                ...
                <br />
                <Text as={"h6"}>Out of stock days: {product.out_stock}</Text>
                <Text as={"h6"}>
              ...
              <Card padding={"5"}>
                ...
                <Text as={"h6"}>
                  ...
                  {parseFloat(
                    // @ts-ignore
                    (product.buy / product.total) * 100
                  ).toFixed(2)}{" "}
                  %
                </Text>
                <Text as={"h6"}>Total sale: {product.buy || 0}</Text>
                <Text as={"h6"}>&nbsp;</Text>
              </Card>
            </HorizontalStack>
          </VerticalStack>
